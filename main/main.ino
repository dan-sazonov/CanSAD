...
#define radio_vcc_pin A0 // пин питания радиомодуля
...
uint32_t srv_last_time = 0;
bool srv_state = false;
bool srv_timeout = false;

int photo_val = 0;

uint32_t radio_last_time = 0;
...
void loop() {
  ...
  if (rec_flag && !srv_state) {
    ...
    srv_last_time = millis();
    digitalWrite(srv_pin, srv_state);
  }
  if (millis() - srv_last_time >= srv_high_time && srv_state) {
    srv_state = false;
    srv_timeout = true;
  }

  // отправляем телеметрию
  if (millis() - radio_last_time >= 500) {
